...
const ManageTours = () => {
  ...
  const fetchCities = async (regionId) => {
    try {
      const response = await axiosInstance.get(`/cities/by-region/${regionId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTourId) {
        await axiosInstance.put(`/tours/update/${editingTourId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        alert("Tour updated successfully!");
      } else {
        await axiosInstance.post(`/tours/add`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleChangeStatus = async (id, currentStatus) => {
    ...
    try {
      await axiosInstance.patch(`/tours/status/${id}?status=${newStatus}`);
        ...
        <tbody>
          {tours.map((tour) => (
            <tr key={tour.id}>
              ...
              <td>
                <img
                  src={`${config.serverUrl}/uploads/${tour.photoPath}`}
                  alt={tour.title}
                  width="100"
                />
              </td>
              <td className="actions">
                <button
                  onClick={() => handleChangeStatus(tour.id, tour.status)}>
                  {tour.status === "ACTIVE" ? "Deactivate" : "Activate"}
                </button>
                <button onClick={() => handleUpdateTour(tour)}>
                  Update Tour
                </button>
              </td>
            </tr>
